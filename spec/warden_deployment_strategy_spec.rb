...
module CfDeployer
  describe WardenDeploymentStrategy do
    ...
    describe "#deploy!" do
      ...
      it "generates the manifest with the correct director UUID" do
        subject.deploy!
        expect(manifest.stubs).to have(3).items
        expect(YAML.load_file(manifest.stubs.last)).to include(
          "director_uuid" => bosh.director_uuid
        )
      end

      it "generates the manifest using the stubs" do
        subject.deploy!
        expect(manifest.stubs).to include(generic_stub, shared_secrets)
      end
